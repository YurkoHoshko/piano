defmodule PianoWeb.Admin.AgentsPage do
  ...
  def handle_event("save", %{"form" => form_params}, socket) do
    ...
    case agent |> Ash.Changeset.for_update(:update_config, attrs) |> Ash.update() do
      {:ok, updated_agent} ->
        ...
        {:noreply,
         socket
         |> assign(agent: updated_agent, form: form, agents: load_agents())
  ...
  defp update_agent_field(socket, field, value) do
    ...
    case agent |> Ash.Changeset.for_update(:update_config, attrs) |> Ash.update() do
      {:ok, updated_agent} ->
        form = build_form(updated_agent)
        {:noreply, assign(socket, agent: updated_agent, form: form, agents: load_agents())}
    ...
    <div class="row">
      <div class="col">
        <section class="card">
          <header>
            <h2>Agents</h2>
            <p>Configure AI agents for Piano.</p>
          </header>

          <%= if @agents == [] do %>
            <p>No agents configured. Run seeds to create a default agent.</p>
          <% else %>
            <table class="table">
              <thead>
                <tr>
              ...
              <tbody>
                <%= for agent <- @agents do %>
                  <tr>
                    ...
                    <td>
                      <a href={"/dashboard/agents?agent_id=#{agent.id}"}>Edit</a>
                    </td>
                  </tr>
                <% end %>
              </tbody>
      ...
      <div class="row">
        <div class="col">
          <section class="card">
            <header>
              <h2>Edit Agent</h2>
              <p>Update settings, tools, and skills.</p>
            </header>

            <.form for={@form} phx-change="validate" phx-submit="save">
              <div class="field">
                <label>Name</label>
                <input type="text" name="form[name]" value={@form[:name].value} required />
              </div>

              <div class="field">
                <label>Model</label>
                <input type="text" name="form[model]" value={@form[:model].value} required />
              </div>

              <div class="field">
                <label>Description</label>
                <textarea name="form[description]" rows="2"><%= @form[:description].value %></textarea>
              </div>

              <div class="field">
                <label>System Prompt</label>
                <textarea name="form[system_prompt]" rows="4"><%= @form[:system_prompt].value %></textarea>
              </div>
              ...
              <div class="field">
                ...
                <%= if @available_skills == [] do %>
                  <p>No skills available. Add .md files to .agents/skills/ directory.</p>
                <% else %>
                  <ul>
              ...
              <div class="field">
                ...
                <a href="/dashboard/agents">Clear selection</a>
              </div>
            </.form>
          </section>
        </div>
